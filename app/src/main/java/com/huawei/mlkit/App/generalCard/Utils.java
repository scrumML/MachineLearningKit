...
package com.huawei.mlkit.App.generalCard;
...
/**
 * Defining post-processing rules and methods for Exit-Entry Permit for Travelling to and from Hong Kong and Macao, Hong Kong Permanent Identity Card and Home Return Certificate (for Compatriots from Hong Kong and Macao).
 */
public class Utils {
    private static Map<String, String> letterNumberMap = new HashMap<>();
    private static Map<String, String> numberLetterMap = new HashMap<>();

    // Misrecognized letters and numbers.
    static {
        Utils.letterNumberMap.put("i", "1");
        Utils.letterNumberMap.put("I", "1");
        Utils.letterNumberMap.put("o", "0");
        Utils.letterNumberMap.put("O", "0");
        Utils.letterNumberMap.put("z", "2");
        Utils.letterNumberMap.put("Z", "2");

        Utils.numberLetterMap.put("1", "I");
        Utils.numberLetterMap.put("0", "O");
        Utils.numberLetterMap.put("2", "Z");
        Utils.numberLetterMap.put("8", "B");
    }
    ...
    public static String getCorrectDate(String origin, String splitter, int[] formatter) {
        ...
        String newStr = Utils.correctLetterToNumber(origin);
        newStr = Utils.filterString(newStr, "[^0-9,.-]");
        ...
        return Utils.fixMissingDelimiter(newStr, target, formatter);
    }
    ...
    public static String fixMissingDelimiter(String origin, char target, int[] format) {
        ...
        // After removing the symbol, it is illegal if smaller than the size of the format.
        String temp = Utils.filterString(origin, "[^0-9a-zA-Z]");
        ...
        for (int i = 0; i < format.length; i++) {
            ...
            if (i != format.length - 1) {
                if (Utils.blurMatchDelimiter(oldChars[oldIndex], target)) {
                    oldIndex++;
    ...
    public static String getCorrectValidDate(String origin) {
        ...
        if (newStr.split(" ").length == 2) {
            // Abnormal conditions, missing delimiters between validity periods.
            strings = newStr.split(" ");
        }
        ...
        int[] formatter = {4, 2, 2};
        String startDate = Utils.getCorrectDate(strings[0], "\\.", formatter);
        String endDate = Utils.getCorrectDate(strings[1], "\\.", formatter);
    ...
    public static String getPassCardNumber(String origin) {
        ...
        newStr = Utils.filterString(newStr, "[^0-9A-Z<]");
        ...
        return Utils.fixMissingDelimiter(newStr, '<', formatter);
    }

    // Obtaining a permanent identity card number for Hong Kong residents.
    public static String getHKIdCardNum(String origin) {
        if (origin == null || origin.isEmpty()) {
            return "";
        }

        origin = Utils.filterString(origin, "[^0-9a-zA-Z()]");
        ...
        number = Utils.correctLetterToNumber(number);
    ...
    // Get Home Return Certificate (for Compatriots from Hong Kong and Macao) number.
    public static String getHomeCardNumber(String origin) {
        if (origin == null || origin.isEmpty()) {
            return "";
        }

        origin = Utils.filterString(origin, "[\\s]");
        if (origin.length() != 8 && origin.length() != 9) {
            return "";
        }

        // Determine if the first character is a letter.
        ...
        if (Character.isLetter(firstLetter.charAt(0))) {
            if (firstLetter.equalsIgnoreCase("H") || firstLetter.equalsIgnoreCase("M")) {
                ...
                String number = origin.substring(1, origin.length());
                number = Utils.filterString(number, "[^0-9]");
                number = Utils.correctLetterToNumber(number);
                if (number.length() != 8) {
                    return "";
                }
        ...
        // Home Return Certificate (for Compatriots from Hong Kong and Macao) only contains number.
        if (origin.length() != 8) {
            return "";
        }
    ...
    public static String correctLetterToNumber(String str) {
        ...
        for (int index = 0; index < length; index++) {
            String tmp = Utils.letterNumberMap.get(Character.toString(chars[index]));
            if (tmp != null) {
    ...
    public static String correctNumberToLetter(String str) {
        ...
        int length = chars.length;
        for (int index = 0; index < length; index++) {
            String tmp = Utils.numberLetterMap.get(Character.toString(chars[index]));
            if (tmp != null) {
                char[] tempChars = tmp.toCharArray();
                chars[index] = tempChars[0];
            }
